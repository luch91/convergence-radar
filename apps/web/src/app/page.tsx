...
async function getSignals(): Promise<{ signals: Signal[]; state: "ready" | "payment" | "disabled" | "unavailable" }> {
  try {
    const response = await fetch(`${apiBaseUrl}/v1/crossings`, { cache: "no-store" });
    if (response.status === 402) return { signals: [], state: "payment" };
    if (response.status === 503) return { signals: [], state: "disabled" };
...
export default async function HomePage() {
  ...
  return <main className="shell">
    ...
    <section id="top" className="hero"><p className="eyebrow">X LAYER INTELLIGENCE</p><h1>Signals with clear limits.</h1><p className="lede">Track tokens where four or more independent tagged wallets buy within 48 hours. The current deployment uses fixture data and identifies each signal as unverified.</p><div className="hero-actions"><a className="primary" href="#signals">View active signals</a><a className="secondary" href="#method">Read the method</a></div></section>
    ...
    <section id="signals" className="section"><div className="section-head"><div><p className="eyebrow">LIVE FEED</p><h2>Active convergences</h2></div><span className={`pill ${state}`}>{state === "ready" ? `${signals.length} available` : state === "payment" ? "payment required" : state === "disabled" ? "payment disabled" : "API unavailable"}</span></div>
      {state === "ready" && signals.length > 0 ? <div className="grid">{signals.map((signal) => <article className="card" key={signal.token}><div className="card-top"><span className="token">{shortAddress(signal.token)}</span><span className="verify">{signal.verificationStatus}</span></div><strong>{signal.buyerCount} independent buyers</strong><p>Window closed {new Date(signal.windowEnd).toLocaleString("en-US", { dateStyle: "medium", timeStyle: "short" })}.</p><a href={`${apiBaseUrl}/v1/token?address=${signal.token}`}>Open paid detail</a></article>)}</div> : <div className="empty"><h3>{state === "payment" ? "Unlock the signal feed" : state === "disabled" ? "The signal feed is disabled" : "The signal feed is unavailable"}</h3><p>{state === "payment" ? "This route is protected by an x402 payment challenge. A compatible client pays 0.5 USDT0 on X Layer and then receives the data." : state === "disabled" ? "The public API has payment processing disabled. It does not return protected signal data." : "Check the API health endpoint, then try again."}</p></div>}
    </section>
